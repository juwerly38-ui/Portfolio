...
|------|----------|------|
| 일반 신청 | 없음 (상시 오픈) | 초록 |
| 긴급 신청 | D+3 영업일 이전 잠금 🔒 | 노랑 |

**D+3 계산 규칙**: 오늘부터 순방향으로 `주말(토/일) + 한국 공휴일`을 건너뛰어 3 영업일 이후 날짜.  
일본 공휴일은 제외하지 않음.

```
예) 오늘 수요일, 목요일=한국 공휴일 → D+3 = 다음주 화요일
예) 오늘 수요일, 목요일=일본 공휴일 → D+3 = 다음주 월요일
```

### 2.4 배정 및 승인 로직
1. 신청자 예약 요청 → `reservations.status = pending`
2. 관리자 승인 시:
